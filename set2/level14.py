...
PRE = ''.join(chr(random.randint(0, 0xff)) for i in range(random.randint(0, 15)))
...
def find_init_offset(block_size):
    block = "1"*16
...
def aes_ecb_oracle_advanced():
    ...
    if not detect_ecb(block_size):
        ...
        return
    print "ECB Detected!"
    init, offset = find_init_offset(block_size)
    i = 1
    known = ""
    while i <= 16:
        for j in string.printable:
            ptxt = "A"*init + "A" * (block_size - i) + known + j + "A"* (block_size-i)
            ctxt = blackbox_encrypt(ptxt)
            blocks = []
            for l in range(0, len(ctxt), block_size):
                blocks.append(ctxt[l:l+block_size])
            if blocks[offset] == blocks[offset+1]:
                known += j
                i = i+1
                break
    while True:
        found_flag = 0
        for j in string.printable:
            ptxt = "A"*init + known[-(block_size-1):] + j + "A" * (block_size-(i % block_size))
            ctxt = blackbox_encrypt(ptxt)
            blocks = []
            for l in range(0, len(ctxt), block_size):
                blocks.append(ctxt[l:l+block_size])
            if blocks[offset] == blocks[offset + (i / block_size) + 1]:
                known += j
                i = i+1
                found_flag = 1
                break
